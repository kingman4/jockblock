...
describe('Cart', () => {
  ...
  describe('initialization', () => {
    ...
    test('loads existing cart from localStorage', () => {
      localStorage.setItem('jockblock-cart', JSON.stringify({
        items: [{ id: 'jockblock-60ml', quantity: 2, price: 19.99 }]
      }));
  ...
  describe('addItem', () => {
    ...
    test('persists to localStorage', () => {
      ...
      expect(stored.items[0].id).toBe('jockblock-60ml');
    });

    test('rejects invalid quantity', () => {
      expect(() => cart.addItem('jockblock-60ml', 0, 19.99))
        .toThrow('Quantity must be at least 1');
      expect(() => cart.addItem('jockblock-60ml', -1, 19.99))
        .toThrow('Quantity must be at least 1');
    });

    test('rejects invalid price', () => {
      expect(() => cart.addItem('jockblock-60ml', 1, -5))
        .toThrow('Price must be positive');
      expect(() => cart.addItem('jockblock-60ml', 1, 0))
        .toThrow('Price must be positive');
    });
  });
  ...
  describe('updateQuantity', () => {
    ...
    test('persists update to localStorage', () => {
      cart.addItem('jockblock-60ml', 1, 19.99);
      cart.updateQuantity('jockblock-60ml', 3);
  ...
  describe('getItemCount', () => {
    ...
    test('returns quantity for single item', () => {
      cart.addItem('jockblock-60ml', 3, 19.99);
  ...
  describe('getItem', () => {
    test('returns item by id', () => {
      ...
      const item = cart.getItem('jockblock-60ml');
      expect(item).toEqual({
        id: 'jockblock-60ml',
        quantity: 2,
        price: 19.99
